...
export class Item {
    ...
    constructor(id: string, filename: string, hash?: string, preview?: string, url?: string) {
        this.id = id;
            ...
            try {
                ...
                if (200 !== req.status) {
                    ...
                    if (file.size <= stop || response.finished) {
                        ...
                        resolve(new Item(response.fid, file.name, response.hash, response.preview, response.url));
                    } else {
                        if (context.onUpdate) {
